...
import {
    CheckCustomPolicy, CheckCustomPolicyModifier,
    FileDialog, GetCategories,
    GetServiceNames,
    RunPasswordModifier,
...
export default function Page() {
    ...
    async function useCustomPreset() {
        if (customFilePath === "") {
            await FileDialog("ats").then(async (result) => {
                ...
                CheckCustomPolicyModifier(result)
                    .then(() => {
                        if (result !== "") {
    ...
    function Run() {
        ...
        RunPasswordModifier(filePath, selectedValue, customFilePath).then((result) => {
            addToArray(setLines, result.Lines);
            addToArray(setPassed, result.Modified);
    ...
    return (
        <Layout isRunning={status === "Processing..."}>
            <GridContainer cols={3} customGap={3} inline={true}>
                ...
                <Chart title={"Modified"} scores={passed}/>
            </GridContainer>
            <div className={`px-2`}>
                <Divider/>
            </div>
